...
#define SHMOO_INCREMENT_10KB  (10 * (1 << 10))		//10 KB
...
void testBandwidthShmoo(memcpyKind kind, printMode printmode, memoryMode memMode, int startDevice, int endDevice, bool wc)
{
    ...
    for (int currentDevice = startDevice; currentDevice <= endDevice; currentDevice++) {
		...
		{
            if (memSize < SHMOO_LIMIT_20KB) { memSize += SHMOO_INCREMENT_1KB; }
            else if (memSize < SHMOO_LIMIT_50KB) { memSize += SHMOO_INCREMENT_2KB; }
            else if (memSize < SHMOO_LIMIT_100KB) { memSize += SHMOO_INCREMENT_10KB; }
            else if (memSize < SHMOO_LIMIT_1MB) { memSize += SHMOO_INCREMENT_100KB; }
            else if (memSize < SHMOO_LIMIT_16MB) { memSize += SHMOO_INCREMENT_1MB; }
            else if (memSize < SHMOO_LIMIT_32MB) { memSize += SHMOO_INCREMENT_2MB; }
            else { memSize += SHMOO_INCREMENT_4MB; }
...
void printHelp(void)
{
    ...
    printf("--device=[deviceno]\tSpecify the device to be used\n");
